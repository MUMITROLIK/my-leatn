...
export const Sidebar = () => {
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex flex-col w-60 shrink-0 border-r border-slate-200 bg-white p-5 sticky top-0 h-screen">
        <div className="flex items-center gap-2 mb-10 px-2">
          <span className="w-10 h-10 rounded-2xl bg-[#58CC02] flex items-center justify-center shadow-md">
            <Sparkles className="w-6 h-6 text-white fill-white" />
          </span>
          <span className="font-display font-bold text-xl text-slate-800">CodeQuest</span>
        </div>
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 z-40 flex justify-around py-2">
        {items.map((it) => (
          <NavLink
            key={it.to}
            to={it.to}
